#include <Arduino.h>
#include <ArduinoJson.h>

// ==========================================
// VARIAVEIS DE ESTADO E SIMULAÇÃO
// ==========================================
String inputBuffer = "";
bool streamActive = false;
unsigned long lastStreamTime = 0;
unsigned long streamInterval = 1000; // Padrão 1s

// Configurações fictícias (Persistência simulada)
float calib_sds = 1.0;
float calib_mq2 = 9.8;
float calib_mq7 = 15.2;
float calib_temp = -1.0;
float calib_hum = 2.0;

// ==========================================
// SETUP
// ==========================================
void setup() {
  // Inicializa Serial com baudrate alto para garantir velocidade no JSON
  Serial.begin(115200);
  
  // Reserva memória para string para evitar fragmentação
  inputBuffer.reserve(200);
  
  randomSeed(analogRead(0));
}

// ==========================================
// LOOP PRINCIPAL
// ==========================================
void loop() {
  // 1. Leitura da Serial (Não bloqueante)
  while (Serial.available()) {
    char c = (char)Serial.read();
    if (c == '\n') {
      inputBuffer.trim(); // Remove \r e espaços extras
      processCommand(inputBuffer);
      inputBuffer = "";
    } else {
      inputBuffer += c;
    }
  }

  // 2. Processamento do Stream (Se ativo)
  if (streamActive) {
    if (millis() - lastStreamTime >= streamInterval) {
      lastStreamTime = millis();
      sendSensorData("ALL");
    }
  }
}

// ==========================================
// PROCESSADOR DE COMANDOS
// ==========================================
void processCommand(String cmd) {
  cmd.toUpperCase(); // Padroniza para caixa alta para facilitar comparação

  // --- COMANDOS GET DATA ---
  if (cmd == "GET DATA" || cmd == "GET DATA ALL") {
    sendSensorData("ALL");
  }
  else if (cmd == "GET DATA SDS011") {
    sendSensorData("SDS011");
  }
  else if (cmd == "GET DATA MQ2") {
    sendSensorData("MQ2");
  }
  else if (cmd == "GET DATA MQ7") {
    sendSensorData("MQ7");
  }
  else if (cmd == "GET DATA DHT") {
    sendSensorData("DHT");
  }
  
  // --- COMANDOS STREAM ---
  else if (cmd == "START STREAM") {
    streamActive = true;
    sendAck("start_stream", "active");
  }
  else if (cmd == "STOP STREAM") {
    streamActive = false;
    sendAck("stop_stream", "idle");
  }

  // --- COMANDOS DE SETTINGS ---
  else if (cmd == "GET SETTINGS") {
    sendSettings();
  }
  else if (cmd.startsWith("SET INTERVAL ")) {
    // Ex: SET INTERVAL 2000
    int newVal = cmd.substring(13).toInt();
    if (newVal > 100) { // Validação simples
      streamInterval = newVal;
      sendAckValue("set_interval", newVal);
    }
  }
  else if (cmd.startsWith("SET CALIB ")) {
    handleCalibration(cmd);
  }

  // --- STATUS E METADATA ---
  else if (cmd == "GET STATUS") {
    sendStatus();
  }
  else if (cmd == "GET METADATA") {
    sendMetadata();
  }
  
  // --- COMANDO DESCONHECIDO ---
  else if (cmd.length() > 0) {
    // Opcional: Enviar erro se desejar
  }
}

// ==========================================
// GERADORES DE RESPOSTA JSON
// ==========================================

// Gera dados aleatórios mas suaves (usando senoide) para testar gráficos
float simulateValue(float base, float variance, float speed) {
  return base + (sin(millis() / speed) * variance) + ((random(-10, 10) / 10.0));
}

void sendSensorData(String target) {
  JsonDocument doc;
  
  doc["type"] = "data";
  doc["src"] = "serial";
  
  // Se for específico, adiciona a tag do sensor
  if (target != "ALL") {
    // CORREÇÃO AQUI: Converte primeiro, atribui depois
    target.toLowerCase();
    doc["sensor"] = target;
  }

  JsonObject payload = doc["payload"].to<JsonObject>();

  // Simulação de Valores
  float temp = simulateValue(26.0, 2.0, 5000.0); 
  float hum = simulateValue(60.0, 10.0, 8000.0);
  float pm25 = simulateValue(30.0, 20.0, 3000.0);
  
  // Preenchimento condicional do Payload
  if (target == "ALL" || target == "sds011") { // target ja foi convertido para minusculo se não for ALL
    payload["pm25"] = round(pm25 * 10) / 10.0;
    payload["pm10"] = round((pm25 * 1.5) * 10) / 10.0; 
  }
  
  if (target == "ALL" || target == "mq2") {
    payload["lpg_ppm"] = (int)simulateValue(200, 50, 4000);
    if (target == "mq2") payload["raw_val"] = random(1400, 1500);
  }
  
  if (target == "ALL" || target == "mq7") {
    payload["co_ppm"] = round(simulateValue(0.5, 0.4, 10000) * 100) / 100.0;
    if (target == "mq7") payload["raw_val"] = random(700, 900);
  }
  
  if (target == "ALL" || target == "dht") {
    payload["temp_c"] = round(temp * 10) / 10.0;
    payload["humid_p"] = round(hum * 10) / 10.0;
  }

  serializeJson(doc, Serial);
  Serial.println(); // Importante: Delimitador
}

void sendAck(String cmd, String status) {
  JsonDocument doc;
  doc["type"] = "ack";
  doc["cmd"] = cmd;
  doc["status"] = status;
  serializeJson(doc, Serial);
  Serial.println();
}

void sendAckValue(String cmd, int val) {
  JsonDocument doc;
  doc["type"] = "ack";
  doc["cmd"] = cmd;
  doc["new_val"] = val;
  doc["status"] = "saved";
  serializeJson(doc, Serial);
  Serial.println();
}

void sendSettings() {
  JsonDocument doc;
  doc["type"] = "settings";
  doc["device_id"] = "AIR_STATION_SIMULATOR";
  doc["interval_ms"] = streamInterval;
  
  JsonObject wifi = doc["wifi"].to<JsonObject>();
  wifi["ssid"] = "AndroidAP_Sim";
  wifi["ip"] = "0.0.0.0";

  JsonObject calib = doc["calib"].to<JsonObject>();
  calib["sds_factor"] = calib_sds;
  calib["mq2_ro"] = calib_mq2;
  calib["mq7_ro"] = calib_mq7;
  calib["temp_offset"] = calib_temp;
  calib["hum_offset"] = calib_hum;

  serializeJson(doc, Serial);
  Serial.println();
}

void handleCalibration(String cmd) {
  // Parsing: SET CALIB SDS 1.1
  int firstSpace = cmd.indexOf(' ');
  int secondSpace = cmd.indexOf(' ', firstSpace + 1);
  int thirdSpace = cmd.indexOf(' ', secondSpace + 1);

  if (secondSpace == -1 || thirdSpace == -1) return;

  String target = cmd.substring(secondSpace + 1, thirdSpace);
  String valStr = cmd.substring(thirdSpace + 1);
  float val = valStr.toFloat();

  if (target == "SDS") calib_sds = val;
  else if (target == "MQ2") calib_mq2 = val;
  else if (target == "MQ7") calib_mq7 = val;
  else if (target == "TEMP") calib_temp = val;
  else if (target == "HUM") calib_hum = val;

  JsonDocument doc;
  doc["type"] = "ack";
  doc["cmd"] = "set_calib";
  
  // CORREÇÃO AQUI: Converte primeiro, atribui depois
  target.toLowerCase();
  doc["target"] = target;
  
  doc["new_val"] = val;
  doc["status"] = "saved";
  
  serializeJson(doc, Serial);
  Serial.println();
}

void sendStatus() {
  JsonDocument doc;
  doc["type"] = "status";
  doc["uptime_sec"] = millis() / 1000;
  doc["wifi_status"] = "disconnected"; // Modo Serial
  
  JsonObject sensors = doc["sensors"].to<JsonObject>();
  sensors["sds011"] = "ok";
  sensors["mq2"] = (millis() < 10000) ? "warming_up" : "ok";
  sensors["mq7"] = (millis() < 10000) ? "warming_up" : "ok";
  sensors["dht11"] = "ok";

  serializeJson(doc, Serial);
  Serial.println();
}

void sendMetadata() {
  JsonDocument doc;
  doc["type"] = "metadata";
  
  JsonArray sensors = doc["sensors"].to<JsonArray>();
  
  JsonObject s1 = sensors.add<JsonObject>();
  s1["id"] = "pm25"; s1["name"] = "PM 2.5"; s1["unit"] = "ug/m3";
  
  JsonObject s2 = sensors.add<JsonObject>();
  s2["id"] = "pm10"; s2["name"] = "PM 10"; s2["unit"] = "ug/m3";

  JsonObject s3 = sensors.add<JsonObject>();
  s3["id"] = "lpg_ppm"; s3["name"] = "GLP"; s3["unit"] = "ppm";
  
  JsonObject s4 = sensors.add<JsonObject>();
  s4["id"] = "co_ppm"; s4["name"] = "Monoxido Carbono"; s4["unit"] = "ppm"; // Corrigido ID duplicado anterior

  JsonObject s5 = sensors.add<JsonObject>();
  s5["id"] = "temp_c"; s5["name"] = "Temperatura"; s5["unit"] = "C";

  JsonObject s6 = sensors.add<JsonObject>();
  s6["id"] = "humid_p"; s6["name"] = "Umidade"; s6["unit"] = "%";

  serializeJson(doc, Serial);
  Serial.println();
}