...
/**
 * @brief Construtor: Inicializa o leitor serial e mapeia os sensores virtuais.
 */
...
/**
 * @brief Configura o callback do Framework e inicia a thread de leitura serial.
 * @param halProxyCallback Interface de comunicação fornecida pelo Android.
 */
...
/**
 * @brief Coleta metadados de todos os sensores instanciados para o framework.
 */
...
/**
 * @brief Ativa/Desativa sensores e gerencia o estado de polling do hardware.
 */
...
/**
 * @brief Define parâmetros de amostragem para um sensor específico.
 */
...
/**
 * @brief Ponto de processamento de dados brutos do hardware.
 * Converte dados da serial em eventos HIDL e os despacha via postEvents.
 */
...
/// @brief Injeção de dados não suportada nesta implementação física.
Return<Result> AirQualitySubHal::injectSensorData(const Event& /*event*/) { return Result::INVALID_OPERATION; }

/**
 * @brief Notifica o framework que o buffer de um sensor foi esvaziado.
 */
...
// HIDL Stubs (Implementação mínima obrigatória)
...
extern "C" {
    ...
    /**
     * @brief Ponto de entrada (Entry Point) da biblioteca carregada pelo Proxy da HAL.
     * @param version Ponteiro de saída para a versão da API (SUB_HAL_2_0_VERSION).
     * @return ISensorsSubHal Ponteiro para a instância singleton da sub-HAL.
     */
    ...
    android::hardware::sensors::V2_0::implementation::ISensorsSubHal* sensorsHalGetSubHal(uint32_t* version) {
        ...
        // Singleton: Garante uma única instância (acesso ao recurso Serial)
